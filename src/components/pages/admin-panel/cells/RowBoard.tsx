...
function RowBoard({ cell, line,teamLeader }: RowBoardProps) {
  const [openPopover, setOpenPopover] = useState(false);
  const handleOpenChange = (newOpen: boolean) => {
    setOpenPopover(newOpen);
  };
  return (
    <div className="grid grid-cols-3 border-b py-3 px-5 border-main_color relative">
      <div className="font-medium text-main_gray">#{cell}</div>
      <div className="text-main_text_color">{teamLeader}</div>
